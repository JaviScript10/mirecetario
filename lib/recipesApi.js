...
export async function duplicateRecipe(recipe) {
  // 1. Obtenemos la sesión activa directamente desde Supabase para no fallar con RLS
  const { data: { user } } = await supabase.auth.getUser();
  if (!user) throw new Error("Usuario no autenticado");

  // 2. Preparamos el borrador de la copia con los nombres del cliente
  const draft = {
  ...
  // 3. Insertamos usando el ID autenticado actual
  return await createRecipe(draft, user.id);
}