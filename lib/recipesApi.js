...
export async function duplicateRecipe(recipe) {
  // 1. Preparamos el borrador de la copia con los nombres que usa la App en el cliente
  const draft = {
  ...
  // 2. Obtenemos el ID del usuario actual o el de la receta
  const userId = recipe.user_id || recipe.userId;

  // 3. Reutilizamos createRecipe para que guarde en Supabase con el mapeo exacto de tu BD
  return await createRecipe(draft, userId);
}